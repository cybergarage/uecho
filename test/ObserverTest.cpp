...
BOOST_AUTO_TEST_CASE(ObjserverListAdd)
{
  ...
  BOOST_REQUIRE_EQUAL(uecho_object_property_observerlist_size(observers), 0);

  for (int n = 0; n < 10; n++) {
    BOOST_REQUIRE(uecho_object_property_observerlist_add(observers, uecho_object_property_observer_new()));
    BOOST_REQUIRE_EQUAL(uecho_object_property_observerlist_size(observers), (n + 1));
  }
